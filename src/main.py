...
def latest_versions(session):
    ...
    for a_tag in a_tags:
        link = a_tag.get('href')
        res = re.search(pattern, a_tag.text)
        if res is not None:
            version = res.group(1)
            status = res.group(2)
            result = (link, version, status)
...
def pep(session):
    ...
    for check_row in tqdm(pep_row):
        ...
        one_link_tag = find_tag(check_row, 'a')
        one_pep_link = one_link_tag['href']
        new_pep_url = urljoin(PEP_URL, one_pep_link)
        pep_soup = BeautifulSoup(
            get_response(session, new_pep_url).text, features='lxml')
        pep_tag = find_tag(pep_soup, 'dl', attrs={
            'class': 'rfc2822 field-list simple'})
        full_status_pep = (pep_tag.text).split()
        full_status_pep = full_status_pep[full_status_pep.index("Status:") + 1]
        ...
        try:
            ...
            raise error
    # for categories_status in sum_status:
